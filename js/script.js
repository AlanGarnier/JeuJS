...
var lesMots = ['superman', 'kleenex', 'technologies', 'crypton', 'futsal', 'argent', 'constantine', 'typographique', 'restaurant', 'mangas']; //Un tableau de mot 
  ...
  if(NChance>0 || IsGagner==true )
  {
    if(toucheSelectionner!=null)
    {
      ...
      if(IsTrouver==false && toucheSelectionner.className  =="btn-floating btn-large waves-effect waves-light")
      {
        ...
        var x = 1;
        x+=(7-NChance);
        document.getElementById("imgPendu").src="Images/"+x+".png";
...
Materialize.toast('I am a toast!', 4000);
    function CreationInput(_unMot)
    {
      ...
      ancienmot=_unMot;
      NChance=7;
    document.getElementById("placePendu").innerHTML="<img id='imgPendu' src='Images/1.png' />"
...
//------------------------------------------------------------------------------------------------------
//Fonction pour générer le caroussel
//------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------
var ancienNombre = ;//L'ancien nombre avant de faire la nouvelle partie
var leNombre = ;//Le nombre à trouver
var lesNombres = [];//Tableau de nombres
var IsInit = true; //Le boolean qui permet de savoir si c'est l'initialisation(true) ou New Partie(false)
var key = {};
var nChance;
...
//Le(s) sélecteur(s)
//-------------------------------------------------------------------------------------------------------
var btnNewGame = document.getElementById("btnInconnu");// Le bouton nouvelle partie d'inconnu
//-------------------------------------------------------------------------------------------------------
//Evènement bouton Nouvelle Partie
//-------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------
//Fontion pour générer un nombre aléatoire
//--------------------------------------------------------------------------------------------------------
var nbSelect;

function nbRandom(nbSelect)
{
    return Math.round(Math.random()*nbSelect);//Retourne un nombre compris entre 0 et le nombre selectionner pas l'utilisateur
}
*/
